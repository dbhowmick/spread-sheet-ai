defmodule SpreadSheetAi.Agents.Coordinator do
  @moduledoc """
  ...
  `:subscribed` via `presence_diff` once the agent appears.
      ...
      subs =
        Sagents.Subscriber.subscribe_to_agent(
          subs,
          SpreadSheetAi.Agents.Coordinator.conversation_agent_id(conversation_id),
          tagged: true
        )

  `tagged: true` makes every event arrive as `{:agent, agent_id, event}`
  rather than `{:agent, event}`. See `@subscribe_opts` below.
  ...
  broadcasts, and upgrades the local subs map to `:subscribed`.
      ...
      # LiveView
      case SpreadSheetAi.Agents.Coordinator.ensure_agent_session_running(socket.assigns) do
        {:ok, changes} -> assign(socket, changes)
        {:error, reason} -> ...
      end

      # GenServer
      case SpreadSheetAi.Agents.Coordinator.ensure_agent_session_running(state) do
  ...
  @presence_module SpreadSheetAiWeb.Presence
  ...
  @config %{
    factory_router: SpreadSheetAi.Agents.FactoryRouter,
    agent_persistence: SpreadSheetAi.Agents.AgentPersistence,
    display_message_persistence: SpreadSheetAi.Agents.DisplayMessagePersistence,
    pubsub: {Phoenix.PubSub, SpreadSheetAi.PubSub},
    presence_module: SpreadSheetAiWeb.Presence,
  ...
  `Sagents.Session.ensure_running/3`.
      ...
      SpreadSheetAi.Agents.Coordinator.ensure_agent_session_running(socket.assigns,
        timezone: socket.assigns.timezone
      )
  """
  ...
  `Sagents.Session.resume/4`.
      ...
      SpreadSheetAi.Agents.Coordinator.resume_agent_session(socket.assigns, response,
        timezone: socket.assigns.timezone
      )
  """
  ...
  `Sagents.Session.dismiss/3`.
      ...
      SpreadSheetAi.Agents.Coordinator.dismiss_agent_session(socket.assigns,
        timezone: socket.assigns.timezone
      )
  """
  ...
  **Raises on a node whose registry is unavailable.**
  ...
  A conversation with no viewers lets an idle agent shut down immediately
  instead of waiting out its inactivity timeout.

  Hosts that keep session state should go through
  `AgentSubscriberSession.add_tracked_viewer/3`, or
  `sync_tracked_viewers/2`, rather than calling this directly, so the entries
  the session records stay truthful about what Presence holds. Calling this
  yourself is how a hand-rolled call site that tracks and never releases gets
  added.
  """
  @impl Sagents.ViewerPresence
  ...
  As with `track_conversation_viewer/3`, hosts that keep session state should go
  through `AgentSubscriberSession.remove_tracked_viewer/2` (or
  `clear_tracked_viewers/1`) so the recorded entries and what Presence holds
  cannot drift apart.
  """
  @impl Sagents.ViewerPresence
