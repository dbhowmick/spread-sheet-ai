defmodule SpreadSheetAi.Agents.DisplayMessagePersistence do
  ...
  def save_message(scope, %Message{} = message, context) do
    ...
    if Enum.empty?(display_items) do
      ...
      |> Enum.reduce_while({:ok, []}, fn {item, index}, {:ok, acc} ->
        ...
        case SpreadSheetAi.Conversations.append_display_message(scope, context.conversation_id, attrs) do
          {:ok, display_msg} ->
  ...
  def update_tool_status(scope, :interrupted, %{call_id: call_id, display_text: display_text}, _context) do
    SpreadSheetAi.Conversations.interrupt_tool_call(scope, call_id, %{"display_text" => display_text})
  end
  ...
  def resolve_tool_result(scope, tool_call_id, result_content, _context) do
    SpreadSheetAi.Conversations.resolve_interrupted_tool_result(scope, tool_call_id, result_content)
  end
