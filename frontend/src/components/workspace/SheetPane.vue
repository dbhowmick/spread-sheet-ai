...
/**
 * One sheet, with its toolbar and grid. Shared by `SheetView` (a sheet on its
 * own, UI-5) and by every tab in the workspace (UI-3).
 *
 * It exists as a component, rather than as markup inside `SheetTabs`, because
 * `useSheet()` cannot be called inside a `v-for`: each open tab needs its own
 * ref-counted hold on its own channel.
 */
...
<template>
  <div class="flex h-full min-h-0 flex-col">
    <Empty v-if="status === 'error'" class="h-full">
      ...
      <EmptyContent v-if="$slots['error-action']">
