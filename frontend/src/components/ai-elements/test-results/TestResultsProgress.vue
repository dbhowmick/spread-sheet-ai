...
<template>
  <div v-if="summary" :class="cn('space-y-2', props.class)" v-bind="$attrs">
    <slot>
      <div class="flex h-2 overflow-hidden rounded-full bg-muted">
        <div class="bg-green-500 transition-all" :style="{ width: `${passedPercent}%` }" />
        <div class="bg-red-500 transition-all" :style="{ width: `${failedPercent}%` }" />
      </div>
